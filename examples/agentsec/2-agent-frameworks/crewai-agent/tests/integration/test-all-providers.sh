...
test_provider_with_mode() {
    ...
    if [ "$mode" = "api" ]; then
        ...
        # Gateway mode - check for gateway communication (LLM or MCP gateway)
        # LLM Gateway: "Gateway.*response", "Gateway handled"
        # MCP Gateway: "[MCP GATEWAY]", "MCPGatewayInspector", "gateway handles"
...
# Setup
log_header "CrewAI Agent Integration Tests"
echo ""
